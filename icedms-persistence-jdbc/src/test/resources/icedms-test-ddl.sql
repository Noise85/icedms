...
CREATE TABLE dms_node (id BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1 INCREMENT BY 1) NOT NULL, parent_id BIGINT, title VARCHAR(255) NOT NULL, PRIMARY KEY(id));
CREATE TABLE dms_node_rule (id BIGINT GENERATED BY DEFAULT AS IDENTITY (START WITH 1 INCREMENT BY 1) NOT NULL, node_id BIGINT NOT NULL, rule_id BIGINT NOT NULL, PRIMARY KEY (id));
...
ALTER TABLE dms_node ADD CONSTRAINT FK_dms_node_dms_node FOREIGN KEY (parent_id) REFERENCES dms_node (id);
ALTER TABLE dms_node_rule ADD CONSTRAINT FK_dms_node_rule_dms_node FOREIGN KEY (node_id) REFERENCES dms_node (id);
ALTER TABLE dms_node_rule ADD CONSTRAINT FK_dms_node_rule_index_rule FOREIGN KEY (rule_id) REFERENCES index_type_document_type (id);
ALTER TABLE dms_node_rule ADD CONSTRAINT UK_dms_node_id_rule_id UNIQUE (node_id, rule_id);